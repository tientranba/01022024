...
function checkLogin() {
  ...
  const wElement = document.querySelector('.w');
    ...
    wElement.addEventListener('mouseover', function() {
      wElement.classList.add('hovered'); // Thêm class khi chuột vào
    });

    wElement.addEventListener('mouseout', function() {
      wElement.classList.remove('hovered'); // Xóa class khi chuột rời đi
    });
    ...
    document.addEventListener('mousemove', function (event) {
      const images = document.querySelectorAll('.animated-image1, .animated-image2');
      ...
      images.forEach((image) => {
        ...
        const scale = 1 + (Math.min(distanceX, distanceY) / maxDistance);
